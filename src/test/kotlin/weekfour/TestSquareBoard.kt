...
class TestSquareBoard {
    ...
    fun test15Neighbour() {
        with(createSquareBoard(4)) {
            ...
            Assert.assertEquals(
                neighbourMessage(cell!!, Direction.UP),
                "(1, 3)",
                cell.getNeighbour(Direction.UP).toString()
            )
            Assert.assertEquals(
                neighbourMessage(cell, Direction.DOWN),
                "(3, 3)",
                cell.getNeighbour(Direction.DOWN).toString()
            )
            Assert.assertEquals(
                neighbourMessage(cell, Direction.LEFT),
                "(2, 2)",
                cell.getNeighbour(Direction.LEFT).toString()
            )
            Assert.assertEquals(
                neighbourMessage(cell, Direction.RIGHT),
                "(2, 4)",
                cell.getNeighbour(Direction.RIGHT).toString()
            )
        }
    }
